...
async function renderLog(data) {
    ...
    const aiLong = customData.ai_long_summary;

    if (aiShort || aiLong) {
        const section = getEl('ai-summary-section');
        if (section) {
            section.style.display = '';
            setHTML('ai-summary-content', `
            <div class="ai-disclaimer">
                ...
                <div>
                    ...
                    The AI does not fully understand all the plugin's mechanics and roles and may therefore contain
                    
                    <strong>erroneous or nonsensical statements</strong>. Please read carefully.
                </div>
            </div>
            ${aiShort ? `
                <div class="ai-short-summary">
                    <div class="mono t3" style="font-size:.58rem;margin-bottom:4px;">SHORT SUMMARY</div>
                    <div style="font-size:.88rem;color:var(--t1);font-weight:600;">${escHtml(aiShort)}</div>
                </div>` : ''}
            ${aiLong ? `
                <details ${!aiShort ? 'open' : ''} style="margin-top:${aiShort ? '10px' : '0'}">
                    <summary class="mono t3" style="font-size:.58rem;cursor:pointer;user-select:none;outline:none;">
                        LONG SUMMARY ${aiShort ? '(expand)' : ''}
                    </summary>
                    <div style="font-size:.8rem;color:var(--t2);line-height:1.6;margin-top:6px;white-space:pre-wrap;">${escHtml(aiLong)}</div>
                </details>` : ''}
        `);
        }
    }
